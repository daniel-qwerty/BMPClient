...
open class AlamofireRequestBuilder<T>: RequestBuilder<T> {
        ...
        if fileKeys.count > 0 {
            manager.upload(multipartFormData: { mpForm in
                ...
                case .failure(let encodingError):
                    let error = ErrorResponse.Error(415, nil, encodingError)
        ...
        case is String.Type:
            validatedRequest.responseString(completionHandler: { (stringResponse) in
                ...
                if stringResponse.result.isFailure {
                    let error = ErrorResponse.Error(stringResponse.response?.statusCode ?? 500, stringResponse.data, stringResponse.result.error as Error!)
                    self.processError(error, completion)
                    completion(nil, error)
                    return
                }
        ...
        case is Void.Type:
            validatedRequest.responseData(completionHandler: { (voidResponse) in
                ...
                if voidResponse.result.isFailure {
                    let error = ErrorResponse.Error(voidResponse.response?.statusCode ?? 500, voidResponse.data, voidResponse.result.error!)
                    self.processError(error, completion)
                    completion(nil, error)
                    return
                }
        ...
        case is Data.Type:
            validatedRequest.responseData(completionHandler: { (dataResponse) in
                ...
                if (dataResponse.result.isFailure) {
                    let error = ErrorResponse.Error(dataResponse.response?.statusCode ?? 500, dataResponse.data, dataResponse.result.error!)
                    self.processError(error, completion)
                    completion(nil, error)
                    return
                }
        ...
        default:
            validatedRequest.responseJSON(options: .allowFragments) { response in
                ...
                if response.result.isFailure {
                    let error = ErrorResponse.Error(response.response?.statusCode ?? 500, response.data, response.result.error!)
                    self.processError(error, completion)
                    completion(nil, error)
                    return
                }
                ...
                let error = ErrorResponse.Error(500, nil, NSError(domain: "localhost", code: 500, userInfo: ["reason": "unreacheable code"]))
                self.processError(error, completion)
                completion(nil, error)
            }
        }
    ...
    open func processError(_ error: ErrorResponse, _ completion: @escaping (_ response: Response<T>?, _ error: Error?) -> Void) {}
